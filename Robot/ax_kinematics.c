...
void AX_ROBOT_Kinematics(void)
{
	...
	R_Vel.RT_IY = ((-R_Wheel_A.RT + R_Wheel_B.RT)/2)*1000;
	R_Vel.RT_IW = ((-R_Wheel_A.RT + R_Wheel_B.RT)/2/(MEC_WHEEL_BASE/2+MEC_ACLE_BASE/2))*1000;	
...
//	R_Wheel_B.TG = R_Vel.TG_FX + R_Vel.TG_FW*(TWD_WHEEL_BASE/2);	
	...
	Gesture_pwm = Gesture(roll, R_Imu.GYRO_Y);
	//利用PID算法计算电机PWM值
	R_Wheel_A.PWM =Speed(20*R_Wheel_A.RT,-20*R_Wheel_B.RT) - Gesture_pwm;   //  
	R_Wheel_B.PWM =Speed(20*R_Wheel_A.RT,-20*R_Wheel_B.RT) - Gesture_pwm;  // 
//  R_Wheel_A.PWM =-Gesture_pwm;
//	R_Wheel_B.PWM =-Gesture_pwm; 
//	printf("A:%d B:%f\r\n",R_Wheel_A.PWM, roll);
	//设置电机PWM值
//	R_Wheel_A.PWM = 200;
	printf("%f, %f, %d\r\n",BalanAngle,roll ,R_Wheel_A.PWM);//-Gesture_pwm,  roll
	AX_MOTOR_A_SetSpeed(R_Wheel_A.PWM);//R_Wheel_A.PWM 
	AX_MOTOR_B_SetSpeed(R_Wheel_B.PWM);//R_Wheel_A.PWM
